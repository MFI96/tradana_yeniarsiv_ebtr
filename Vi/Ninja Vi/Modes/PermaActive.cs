...
using Settings = Vi.Config.Modes.MiscMenu;
...
namespace Vi.Modes
{
    public sealed class PermaActive : ModeBase
    {
        ...
        public override void Execute()
        {
            ...
            if (Config.Modes.Combo.MainMenu["flashq"].Cast<KeyBind>().CurrentValue)
            {
                FlashQ();
                return;
            }

            if (Config.Modes.Combo.MainMenu["forceR"].Cast<KeyBind>().CurrentValue)
            {
                ForceR();
                return;
            }
